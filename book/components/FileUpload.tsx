...
const FileUpload = ({ type, accept, placeholder, folder, variant, onFileChange, value }: Props) => {
  ...
  const onValidate = (file: File) => {
    ...
    if (file.size > maxSize) {
      toast.error(`File size too large. Please upload a file that is less than ${maxSize / (1024 * 1024)}MB.`);
      return false;
    }
    return true;
  };
  ...
  return (
    <ImageKitProvider publicKey={publicKey} urlEndpoint={urlEndpoint} authenticator={authenticator}>
      ...
      {file.filePath &&
        (type === "image" ? (
          <IKImage alt={file.filePath} path={file.filePath} className="h-96 w-full rounded-xl" width={500} height={300} />
        ) : type === "video" ? (
